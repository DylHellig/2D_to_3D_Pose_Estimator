...
def translate_image(image, trans_x, trans_y):
    ...
    T = np.float32([[1, 0, trans_x], [0, 1, trans_y]])
    ...
    while True:
        ...
        for step in range(0, num_samples, batch_size):
            ...
            for idx in batch_indices:
                ...
                image = translate_and_rotate(
                    image, translation_x, translation_y, rotation
                )
...
def rot_data_generator(
    volume, first_images, img_filenames, lbl_filenames, heights, batch_size
):
    # center = np.array(volume.shape) // 2
    ...
    while True:
        ...
        for step in range(0, num_samples, batch_size):
            ...
            for idx in batch_indices:
                ...
                translation_y = utils.random_sample(translations, 1)
                rotation = utils.random_sample(rotations, 1)
                # aug_img1 = translate_and_rotate(img1,
                # translation_x, translation_y, rotation)
                aug_img2 = translate_and_rotate(
                    img2, translation_x, translation_y, rotation
                )
